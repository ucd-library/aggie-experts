...
program.name('splay')
  ...
  .option('--context <context>', 'construct query for each binding')
  .option('--context@ <context.json>', 'file containing construct query for each binding')
...
if (cli.fuseki.isTmp) {
  const fuseki = await ec.createFusekiDb(cli, files);
  cli.source ||= [];
