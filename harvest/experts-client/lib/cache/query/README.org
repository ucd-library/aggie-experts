...
* Sparql Queries
Originally, we used this document for some literate programming for the sparql
queries, so that we could more fully document the individual parts of the Sparql
quereies.  However, operationally, it was often quicker to directly modify the
templates, and not this file, and so the explicit connection between these
document and the query templates is no longer used.

However, information in this document is still relevant, and has been updated to
the Version 4 queries, and is useful for understanding the queries.
...
During the harvest, the ~?EDUROAM__~ is filled in with an constant value for
    each expert in the database, typically only one, eg. ~quinn@ucdavis.edu~.
    Though these look like emails, they  are actually the expert's eduroam
    identifier, their CasID plus ~@ucdavis.edu~.
