 ...
 * shape JSON that the API endpoints return. This class centralizes:
 *
 *   - the URI constants used to address ae-std nodes
 *   - small accessors (firstValue / allValues / bool / collapse) that hide the
 *     `[{ "@value": ... }]` wrapping
 *   - the TYPE_COMPACTION table and compactType helper that mirror the ES
 *     JSON-LD context's type-shortening rules
 *   - general JSON-LD plumbing (asArray, hasType, normalizeExpertId,
 *     stripAeBase, partialDateToFull, readJson)
 *
 * Members are static — this class is a namespace. Usage:
 *
 *   import { PgJsonld } from '../pg-jsonld.js';
 *   const name = PgJsonld.firstValue(node, PgJsonld.URI.CSL_TITLE);
 *
 * Keep this module side-effect-free aside from the `cache` import that backs
 * readJson, so it can be imported anywhere without dragging in a database
 * connection.
 */
...
class PgJsonld {
  ...
  // the citation-style (csl) vocabulary — NOT bibo or dcterms — for almost
  // every bibliographic field. The expert (person) nodes use schema.org +
  // vivo + vcard. Mirror that here so the normalizers extract everything
  // cleanly.
  static URI = {
  ...
  static AE_BASE_URL = 'http://experts.ucdavis.edu/';

  static stripAeBase(id) {
    return (typeof id === 'string' && id.startsWith(PgJsonld.AE_BASE_URL))
      ? id.slice(PgJsonld.AE_BASE_URL.length)
      : id;
  }
  ...
  // Type compaction
  // --------------------------------------------------------------------------
  // JSON-LD context-driven type compaction is non-uniform in the ES api
  // output: types with named terms in the webapp context collapse to bare
  // names (Work, Authorship, Name, URL, Title, ScholarlyArticle, …) while
  // unmapped types keep their namespace prefix (vcard:Organization,
  // ucdlib:Authorship, vcard:Individual). Hardcode the mapping to match what
  // ES emits, since we're not running the JSON-LD framing machinery ourselves.
  //
  // To add a new type: run the diff against ES, see how the type appears in
  // the response, and add an entry here. Unmapped types fall back to
  // toShortType, which usually produces a bare name.
  static TYPE_COMPACTION = {
    // Vcard types
  ...
  static compactType(t) {
    if (typeof t !== 'string') return t;
    return PgJsonld.TYPE_COMPACTION[t] || PgJsonld.toShortType(t);
  }

  static toShortPrecision(precision) {
  ...
  static hasType(node, type) {
    return PgJsonld.asArray(node?.['@type']).includes(type);
  }

  /**
   * Pull the first value out of a JSON-LD property. Handles both @value
   * (literal) and @id (reference) wrapping, plus bare scalars. Returns
   * undefined when absent.
   */
  static firstValue(node, uri) {
    const first = PgJsonld.asArray(node?.[uri])[0];
   ...
   */
  static allValues(node, uri) {
    return PgJsonld.asArray(node?.[uri])
   ...
   */
  static bool(node, uri, defaultValue=false) {
    const raw = PgJsonld.firstValue(node, uri);
   ...
   */
  static collapse(node, uri) {
    const values = PgJsonld.allValues(node, uri);
  ...
  // --------------------------------------------------------------------------
  static normalizeExpertId(value) {
  ...
  static pickRoleType(role={}) {
    const types = PgJsonld.asArray(role['@type']);
    return types[0] || null;
  }
   ...
   * or unparseable.
   */
  static partialDateToFull(value) {
    if (!value || typeof value !== 'string') return null;
    const trimmed = value.trim();
    if (!trimmed) return null;
  ...
  // --------------------------------------------------------------------------
  static SCHEMA_NAME = 'api';

  static assertSchemaName(schema) {
