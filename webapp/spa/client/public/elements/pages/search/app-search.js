    ...
    (this.downloads || []).forEach(download => {
      ...
      if( resultType === 'expert' ) {
        ...
        contributors.filter(c => !c['ae-roleof-suppress']).forEach(c => {
          ...
          name = name.trim();
          const normalizedName = utils.getNormalizedContributor(name);
          ...
          if( role === 'Principal Investigator' || role === 'Co-Principal Investigator' ) {
            if( !utils.hasSeenContributor(seenPisCoPis, normalizedName) ) {
              utils.markSeenContributor(seenPisCoPis, normalizedName);
              pisCoPis += name + '; ';
            }
          } else {
            if( !utils.hasSeenContributor(seenOtherContributors, normalizedName) ) {
              utils.markSeenContributor(seenOtherContributors, normalizedName);
