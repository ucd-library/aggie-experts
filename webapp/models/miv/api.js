...
async function grantsHandlerEs(req, res) {
  const params = {};
...
async function rawGrantsHandlerEs(req, res) {
  const params = {};
...
async function grantsHandlerPg(req, res) {
    ...
    try {
      ...
      const out = grants.map(grant => {
        ...
        // Get the queried expert's display name to exclude their unresolved stubs.
        // #roleof_ stubs without inheres_in won't be filtered by the inheres_in check,
        // so compare names directly until harvest resolves them to a proper expert_id.
        const expertDisplayName = cleanContributorName(
          roles.find(r => r.expert_id === normalizedExpertId)?.display_name || ''
        ).toLowerCase();
        ...
        const rawContributors = rawRelatedBy
          ...
          .filter(r => !expertDisplayName || cleanContributorName(r.name).toLowerCase() !== expertDisplayName)
...
async function rawGrantsHandlerPg(req, res) {
    const since = req.query.since || null;
    const until = req.query.until || generateGrantFormattedDate();
    const expertId = String(req.expertId || '').trim();
    const normalizedExpertId = expertId.replace(/^expert\//, '');
...
router.get('/grants',     has_access('miv'), fetchExpertId, usePg ? grantsHandlerPg    : grantsHandlerEs);
router.get('/raw_grants', has_access('miv'), fetchExpertId, usePg ? rawGrantsHandlerPg : rawGrantsHandlerEs);
router.get('/grants_pg',     has_access('miv'), fetchExpertId, grantsHandlerPg);
router.get('/raw_grants_pg', has_access('miv'), fetchExpertId, rawGrantsHandlerPg);
